    ...
    intervals.push(setInterval(() => {
      try {
        if (bot?.entity) {
          ...
          setTimeout(() => bot.setControlState(dir, false), 800 + Math.random() * 400);
...
function startTimedMessages(botId, bot, timedMessages) {
  ...
  timedMessages.forEach(tm => {
    ...
    const totalMs = ((parseInt(tm.hours) || 0) * 3600 + (parseInt(tm.minutes) || 0) * 60) * 1000;
    if (totalMs < 5000) {
      addLog(botId, `⚠️ Skipped timed msg "${tm.message}" — interval too short (min 5s)`, 'warn');
      return;
    }
    const id = setInterval(() => {
      try {
        if (bot?.entity) {
          ...
          const label = tm.message.trim().startsWith('/') ? '⚡ Command' : '📢 Timed msg';
    ...
    addLog(botId, `⏱ Scheduled: "${tm.message.trim()}" every ${hStr}${mStr}`, 'info');
  });
...
function startBot(botConfig) {
  const { id, host, port, username, version, antiAfk, timedMessages } = botConfig;
  ...
  try {
    bot = mineflayer.createBot({
      host, port: parseInt(port, 10), username,
      version: version || false, auth: 'offline', hideErrors: false
    });
  } catch (err) {
  ...
  const runtime = { bot, afkIntervals: [], msgIntervals: [], logs: [] };
...
app.post('/api/bots', authMiddleware, (req, res) => {
  const { name, host, port, username, version, antiAfk, timedMessages } = req.body;
  if (!name || !host || !port || !username)
    return res.status(400).json({ error: 'name, host, port and username required' });
  const bots = readBots();
  const newBot = {
    ...
    antiAfk: {
      jumpEnabled: antiAfk?.jumpEnabled ?? true, jumpInterval: antiAfk?.jumpInterval ?? 30,
      walkEnabled: antiAfk?.walkEnabled ?? true, walkInterval: antiAfk?.walkInterval ?? 45,
      lookEnabled: antiAfk?.lookEnabled ?? true, lookInterval: antiAfk?.lookInterval ?? 20
    },
...
app.put('/api/bots/:id', authMiddleware, (req, res) => {
  ...
  const { name, host, port, username, version, antiAfk, timedMessages } = req.body;
  bots[idx] = { ...bots[idx], name, host, port: parseInt(port, 10), username, version: version || '', antiAfk, timedMessages };
  writeBots(bots);
  const runtime = activeBots.get(req.params.id);
  if (runtime) {
    stopIntervals(runtime.afkIntervals);
    stopIntervals(runtime.msgIntervals);
    runtime.afkIntervals = startAntiAfk(req.params.id, runtime.bot, antiAfk);
    runtime.msgIntervals = startTimedMessages(req.params.id, runtime.bot, timedMessages || []);
    addLog(req.params.id, '⚙️ Settings updated and applied live', 'info');
  }
  res.json(bots[idx]);
});
